...
<html>
	...
	<body>
		...
		<table>
			<%
				PalavraNegadaDAO dao = new PalavraNegadaDAO();
				ArrayList<PalavraNegada> lista = new  ArrayList<PalavraNegada>(); 
				lista = (ArrayList<PalavraNegada>)dao.lista();
				for(int x=0; x<lista.size(); x++){
			%>		
			<tr>
				<td><%=lista.get(x).getPalavra()%></td>
				<td><a href="/Forum/adicionaThread?idThread=<%=lista.get(x).getId()%>">Remover</a></td>
			</tr>
			<%
				}
			%>
